...
class CardSwitcherCardTest < Test::Unit::CardTestCase
  ...
  def test_play_ids_switches_to_music_card
    @card.receive_message Honcho::Message.new(:pass_focus, method: "play_ids", params: "1,2,3")
    assert_kind_of Mozart::MusicCard, @application.cards.last
    assert_equal "1,2,3", @application.cards.last.params
  end

  def test_queue_ids_switches_to_music_card
    @card.receive_message Honcho::Message.new(:pass_focus, method: "queue_ids", params: "1,2,3")
    assert_kind_of Mozart::MusicCard, @application.cards.last
    assert_equal "1,2,3", @application.cards.last.params
  end
  ...
  def test_play_stream_switches_to_internet_radio_card
    ...
    assert_equal "http://test.stream", @application.cards.last.params
  end
